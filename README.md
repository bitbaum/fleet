...
| `scripts/ci/auto-merge-sweep.sh` | the canonical merge policy; the fleet calls it via the reusable workflow below. Includes the contributor gate: an outside PR merges only when every commit is `Signed-off-by` **and** a maintainer approved its head commit, certifying [`bitbaum/.github/CONTRIBUTING.md`](https://github.com/bitbaum/.github/blob/main/CONTRIBUTING.md) — the DCO plus the licence grant that keeps relicensing possible |
