...
| `scripts/ci/currency-reconciler.mjs` | which repos are behind on the fleet's OWN packages — and it OPENS THE PULL REQUEST rather than reporting. Exists because `version-currency.mjs` judges by MAJOR, which is right for `next` and wrong for us: we ship features in minors. Measured 2026-09-21, `@bitbaum/ai-kit` was published at 1.10.0 and ten of its fourteen consumers sat on major 1 with an older minor — so the ratchet was **green** while a fix published for everyone reached two repos. Reporting is what already failed: this file has said "version spread is the metric to watch" and "nobody schedules it" since before the spread was measured, and neither sentence moved a version. Downstream already works without a human — PR → the repo's CI → `auto-merge-sweep` → CD → box. A green bump ships itself; a red one is the useful outcome, naming the repo that cannot take the new version, which nothing else in the fleet would have told us. `@bitbaum/*` ONLY: auto-bumping someone else's major on a schedule is a different risk and our CI is not a safety net for it. Checks the repo out and runs `pnpm update` so the LOCKFILE is written by the package manager — an API-patched manifest fails CI in every repo every week for a reason unrelated to the bump, and a bot whose PRs are always red gets muted. Capped at 20 PRs a run — a number from a measurement, not a feeling: the FIRST LIVE RUN (2026-09-21) swept 43 repos and found **13** wanting a bump, and the cap was 12 because this script's docblock had miscounted the same fleet as eleven. So the guard fired on its first run against a plan that was entirely correct: the right behaviour for the wrong reason, and precisely the false positive that teaches people to raise a cap without reading it. 20 keeps it meaningful — far under the 43 swept, so "the whole fleet at once" still refuses. That run also found drift nobody had measured: `@bitbaum/mail-kit` 0.1.0 → 0.1.1 in five repos, caught because the scope is `@bitbaum/*` rather than ai-kit alone. Self-tested by `scripts/ci/test-currency-reconciler.mjs` — **45 checks, no network, no token, no checkout**, fixtures taken from the real 2026-09-21 spread, pinning both sides: the same-major drift is caught, a current repo stays silent, and `workspace:*` / a git pin / `>=1.2 <2` come back UNTOUCHED. |
