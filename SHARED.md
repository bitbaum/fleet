...
**Adopted:** `listkit` — fleetcrown (proving consumer, 2026-09-11: `/fleet`'s search, four facet rows and four sorts, every control a link or a GET form so the page filters with JavaScript off). Next adopter is whichever repo's hand-rolled filter bar is touched next — hirnli is the richest target, with 8 toggle copies, 11 flag toggles and two live defects the package's own tests would have caught.
